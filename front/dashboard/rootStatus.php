...
<body>
	...
	<div class="card text-center" id="status"> <!--Status Geral-->
	  	...
	  	<div class="card-body">
	  		<div id="tblStatus">
	  			<?php
	  				...
	  				$resGeral=$conn->query($sql);
	  				$linhaGeral=$resGeral->fetch_array();
	  				$sql="select * from alunos;";
	  				$resIndi=$conn->query($sql);
	  				$conn->close();
  				?>
		    	<table class="table"> <!--Geral-->
	  				<thead class="thead-dark">
	    				<tr>
	    					<th scope="col" colspan="2">Dados Gerais</th>
	    				</tr>
					</thead>
		  			<tbody>
						<tr>
							<th scope="col" width="400">Quantidade de Alunos</th>
					      	<td><?php echo $linhaGeral['count(*)'];?></td>
	    				</tr>
	    				<tr>
	    					<th scope="col">Estimativa de Rentabilidade</th>
					      	<td><?php echo "R$ ".(($linhaGeral['count(*)'])*70).",00";?></td>
	    				</tr>
	  				</tbody>
	  			</table>
	  			<table class="table"> <!--Indovidual-->
	  				<thead class="thead-dark">
	    				<tr>
	    					<th scope="col" colspan="4">Dados do Aluno</th>
	    				</tr>
					</thead>
		  			<tbody>
		  				<?php
		  				while($linhaIndi=$resIndi->fetch_array()){
		  				?>
						<tr class="table-active">
							<th scope="col">Nome</th>
					      	<td><?php echo $linhaIndi['nome'];?></td>
					      	<th scope="col">Aula</th>
					      	<td><?php echo $linhaIndi['aula'];?></td>
	    				</tr>
	    				<tr>
	    					<th scope="col">Meses de Aula</th>
					      	<td>
				      		<?php 
				      			$data1 = new DateTime(date('Y/m/d'));
				      			$data2 = new DateTime($linhaIndi['dtEntrada']);
				      			$intervalo = $data1->diff($data2);
				      			echo "{$intervalo->y} (anos) - {$intervalo->m} (meses) - {$intervalo->d} (dias) ";
				      		?>
					      	</td>
					      	<th scope="col">Status Atual</th>
					      	<td><?php echo $linhaIndi['liberacao']==1?"Ativo":"Em Espera";?></td>
	    				</tr>
	    				<?php } ?>
	  				</tbody>
				</table>
				<a class="btn btn-danger btn-block" href="rootStatus.php?program">
                    Programar { }
                </a>
