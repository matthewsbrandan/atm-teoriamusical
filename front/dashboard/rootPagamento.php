...
<body>
	...
	<div class="card text-center" id="pagamento"> <!--Pagamentos-->
	  	<div class="card-header">
	    	Pagamento de Aulas - <?php echo rMes(date('m'));?>
	  	</div>
	  	<div class="card-body">
	  		<div id="tblPg">
	  			<?php
	  				include('../../back/conexao.php');
	  				$conn->query("call atualizaPg();");
	  				$sql="select id,nome,diaPg from alunos where pg='Pendente';";
	  				$resDebito = $conn->query($sql);
	  				$sql="select a.id idaluno,a.nome nome,date_format(p.dataPg,'%d/%m/%Y') data from pagamento p inner join alunos a on p.aluno = a.id where month(dataPg)=date_format(now(),'%m') and a.pg='Em dia';";
	  				$resPago = $conn->query($sql);
	  				$conn->close();
	  			?>
	  			<table class="table" id="pg">
			  		<thead>
				    	<tr>
					      	<th scope="col" colspan="3">Em Débito</th>
				    	</tr>
				  	</thead>
				  	<tbody>
				  		<?php
				  		while($linhaDeb = $resDebito->fetch_array()){
				  		?>
				    	<tr>
				      		<td><?php echo $linhaDeb['nome']." - Vencimento dia ".$linhaDeb['diaPg'];?></td>
				      		<td colspan="2"><a class="hoverWhite" href="../../back/backCadDocument.php?deb=<?php echo $linhaDeb['id'];?>"><i class="material-icons float-right pg" title="Receber Pagamento">attach_money</i></a></td>
				    	</tr>
				    	<?php
			  			}
				  		?>
				  	...
				  	<thead>
				    	<tr>
					      	<th scope="col" colspan="3">Pago</th>
				    	</tr>
				  	</thead>
				  	<tbody>
				  		<?php
				  		while($linhaPG = $resPago->fetch_array()){
				  		?>
				    	<tr>
				      		<td><?php echo $linhaPG['nome'];?></td>
				      		<td style="text-align: right;" width="50"><?php echo $linhaPG['data'];?></td>
				      		<td width="60"><a class="hoverWhite" href="../../back/backCadDocument.php?rem=<?php echo $linhaPG['idaluno'];?>"><i class="material-icons float-right cancel" title="Cancelar Recebimento">close</i></a></td>
				    	</tr>
				    	<?php
				    	}
				  		?>
                ...
                <a class="btn btn-danger btn-block" href="rootPagamento.php?program">
                    Programar { }
                </a>
