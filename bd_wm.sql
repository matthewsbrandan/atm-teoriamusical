drop database if exists bd_wm;
	create database bd_wm;
    ...
    select * from chat;
    select id_usuario,nome,count(*) from chat c inner join usuario u on c.id_usuario=u.id where msg_view=0 group by id_usuario order by c.id desc ;
    select * from usuario;